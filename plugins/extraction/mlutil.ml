...
| MLcons (info1, r1, a1), MLcons (info2, r2, a2) ->
    info1 = info2 && r1 = r2 && list_eq ( =* ) a1 a2
| MLcase (info1, t1, brs1), MLcase (info2, t2, brs2) ->
    info1 = info2 && t1 =* t2
      && list_eq br_equiv (Array.to_list brs1) (Array.to_list brs2)
| MLfix (i1, ids1, a1), MLfix (i2, ids2, a2) ->
    i1 = i2 && ids1 = ids2
      && list_eq ( =* ) (Array.to_list a1) (Array.to_list a2)
...
let needs_magic p = try mgu p; NoMagic with Impossible -> NeedsMagic p
...
(* Remark!: type must not be changed by f *)
...
let nb_occur_match =
  let rec nb k = function
    | MLrel (i, _) -> if i = k then 1 else 0
    | MLcase(_,a,v) ->
        (nb k a) +
	Array.fold_left
...
let rec test_eta_args_lift k n = function
  | [] -> n=0
  | a :: q ->
      (match a with | MLrel (x,_) when x = k+n -> true | _ -> false)
        && (test_eta_args_lift k (pred n) q)
...
let rec annot_map f = function
  ...
  | MLtuple ts -> MLtuple (List.map (annot_map f) ts)
...
let rec ast_glob_subst s t = match t with
  | MLapp ((MLglob ((ConstRef kn) as refe, ty_args)) as f, a) ->
  ...
  | MLglob ((ConstRef kn) as refe, ty_args) ->
...
and simpl_case o typ br e =
  ...
  with Impossible ->
	...
	| Some (f,ints) when Intset.cardinal ints = Array.length br ->
	  ...
	  simpl o (MLletin (Tmp anonymous_name, (0,tydummy), e, f))
	| Some (f,ints) ->
	  let last_br =
...
let term_expunge s (idtys,c) =
  if s = [] then c
    ...
    if idtys = [] && lang () <> Haskell && List.mem (Kill Kother) s then
...
let kill_dummy_args ids r t =
  ...
  let bl = List.rev_map sign_of_id (List.map fst ids) in
  let rec found n = function
    | MLrel (r',_) when r' = r + n -> true
...
let rec kill_dummy = function
  | MLfix(i,fi,c) ->
      (try
	 let ids,c,_ = kill_dummy_fix i c in
	 ast_subst (MLfix (i,fi,c)) (kill_dummy_args ids 1 mlrel1)
  ...
  | MLletin(id, (n,ty), MLfix (i,fi,c),e) ->
      (try
	 let ids,c,ids' = kill_dummy_fix i c in
  ...
  | MLletin(id,(i,ty),c,e) ->
      (try
	 let ids,c,ids' = kill_dummy_lams (kill_dummy_hd c) in
	 let e = kill_dummy (kill_dummy_args ids 1 e) in
...
and kill_dummy_hd = function
  ...
  | MLletin(id,ty,c,e) ->
      (try
	 let ids,c,_ = kill_dummy_lams (kill_dummy_hd c) in
...
and kill_dummy_fix i c =
  ...
  let ids,ci,ids' = kill_dummy_lams (kill_dummy_hd c.(i)) in
